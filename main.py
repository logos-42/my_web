...
def get_modified_files(config: dict) -> List[Path]:
    ...
    # 获取特定目录下的Markdown文件
    md_files = []
    content_dirs = ['articles', 'blogs', 'projects', 'art', 'music']
        ...
        for article in processor.generate_article_list():
            article_html = f'''
                    <div class="essay-item">
                        <a href="{article['link']}" class="essay-title">{article['title']}</a>
    ...
    try:
        ...
        for md_file in md_files:
            try:
                ...
                # 如果是blogs目录下的文件，直接输出到根目录的blogs目录
                if rel_path.parts[0] == 'blogs':
                    output_path = root_dir / 'blog' / rel_path.name.replace('.md', '.html')
                else:
                    output_path = root_dir / rel_path.with_suffix('.html')
