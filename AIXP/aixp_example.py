...
class AgentA:
    ...
    def request_length(self, receiver_url: str, text: str):
        ...
        response = requests.post(receiver_url, data=message.json(), headers=headers)
...
class AgentB:
    ...
    def process_request(self, message_json: str) -> str:
        try:
            message = AIXPMessage.parse_raw(message_json)
...
if __name__ == "__main__":
    ...
    message_to_send = AIXPMessage(
        sender_id=agent_a.agent_id,
        receiver_id=agent_b.agent_id,
        task="get_length",
        data={"text": text_to_analyze}
    )
    response_b_json = agent_b.process_request(message_to_send.json())
    response_b = AIXPMessage.parse_raw(response_b_json)
